...
const VehicleSearch = () => {
  ...
  return (
    <div className='w-full place-content-center space-y-4 mt-8 bg-cover bg-center mb-10 dark:text-white'>
      ...
      <div className="border-b-4 border-black w-full dark:border-white"></div>
      ...
      <select
        ...
        className="mb-3 mr-4 shadow appearance-none border rounded py-2 px-3  leading-tight focus:outline-none focus:shadow-outline dark:bg-navPurpleDarker"
    >
    ...
    >
      ...
      <input
        ...
        className="mb-3 mr-4 shadow appearance-none border rounded py-2 px-3  leading-tight focus:outline-none focus:shadow-outline end-0 dark:bg-navPurpleDarker"
      />
   
      </div>

      <table className='w-full border-collapse border-spacing-2 border-black rounded-md pad shadow-xl p-5 '>
        <thead className='bg-secondary text-white border-white dark:bg-black dark:border-black'>
          <tr>
            <th className='border  p-2 '>Vehicle Category</th>
            <th className='border  p-2 '>Vehicle Type</th>
            <th className='border  p-2 '>Vehicle Model</th>
            <th className='border  p-2 '>Vehicle Register</th>
            <th className='border  p-2 '>Vehicle State</th>
            <th className='border  p-2'>Avalability</th>
            <th className='border  p-2'>Actions</th>
          </tr>
        </thead>
        <tbody>
          {currentChunk.map(vehicle => (
            <tr className="bg-white border-gray-200 dark:bg-navPurpleDarker dark:border-black" key={vehicle._id}>
              ...
              <td className={`px-6 py-2 whitespace-nowrap border-r text-center font-semibold  ${vehicle.statusVehicle === 'Active' ? (availabilityStatuses[vehicle._id] === 'Available' || availabilityStatuses[vehicle._id] === '' ? 'text-green-500 bg-green-100' : 'text-red-600 bg-red-300') : ''}`}>
              ...
              <td className="px-2 py-2 whitespace-nowrap border-r  flex justify-center">
                    {vehicle.statusVehicle === 'Active' && (
                    <>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center ">
          <button className=" bg-gray-200 py-2 px-4   text-xs font-bold hover:bg-gray-300 rounded-s-full dark:bg-black" onClick={handlePrevPage} disabled={currentPage === 0}>Previous</button>
          
          {Array.from(Array(totalPages).keys()).map((pageIndex) => (
            <button key={pageIndex} className={`dark:bg-black bg-null py-2 px-4 bg-gray-200   text-sm font-bold ${pageIndex === currentPage ? 'bg-gray-300' : ''}`} onClick={() => handlePageClick(pageIndex)}>
              {pageIndex + 1}
            </button>
          ))}
          <button className="bg-gray-200 py-2 px-4   text-xs font-bold hover:bg-gray-300 rounded-e-full dark:bg-black" onClick={handleNextPage} disabled={currentPage === totalPages - 4}>Next</button>
        </div>
      )}
