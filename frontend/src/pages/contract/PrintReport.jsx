...
const PrintReport = () => {
  ...
  useEffect(() => {
    const clientContracts = allcontracts.filter(
      (contract) => contract.clientID._id === clientID
    );
    if (clientContracts) {
      const numMade = clientContracts.length;
      const numTerminated = clientContracts.filter(
        (contract) => contract.Status === "Terminated"
      ).length;
      const ongoingCont = clientContracts.find(
        (contract) =>
          contract.Status === "ongoing" || contract.Status === "Newly Added"
      );
  ...
  return (
    <div className="flex flex-col   items-center">
      ...
      <div
        className="bg-[#D9D9D9] w-[90%] h-fit rounded-lg py-4 flex justify-evenly"
        ref={ref}
      >
      ...
      <div>
        <button
          className=" bg-orange-600 px-5 py-2 rounded-lg w-[150px] font-bold text-white mr-4"
          onClick={() => {
            navigate("/Report");
          }}
        >
        ...
        <ReactToPrint
          ...
          content={() => ref.current}
          trigger={() => (
            <button className=" bg-blue-600 px-5 py-2 rounded-lg w-[150px] my-5 text-white font-bold">
              Print
            </button>
          )}
        />
