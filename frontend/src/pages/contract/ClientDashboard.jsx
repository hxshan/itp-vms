...
const ClientDashboard = () => {
  ...
  return (
    <div className="w-full h-full py-5">
        ...
        <button className=" bg-actionBlue text-white px-5 py-2 rounded-xl text-[16px] font-bold  uppercase " onClick={()=>{navigate(`/addClient`)}} >Add client</button>
        </div>
    ...
    .map((row) => {
      return (
        <tr className="bg-white border-t border-gray-200" key={row._id}>
          ...
          <td className="px-6 py-2 whitespace-nowrap justify-center gap-3 flex">
            ...
            <button className="bg-actionGreen text-white py-1 px-6 rounded-md" onClick={()=>{navigate(`/EditClient/${row._id}`)}}>Edit</button>
          </td>
        </tr>
      );
    }) : (
      ...
      <div className="flex justify-center mt-4">
                <ul className="flex list-none border border-gray-300 rounded-md">
                    {Array.from({ length: Math.ceil(allClients.length / itemsPerPage) }).map((_, index) => (
