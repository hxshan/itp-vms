...
const ContractDasboard = () => {
  ...
  useEffect(() => {
    if (clientError) {
      ...
      const availableClients = clients.filter(
        (client) => client.Contract_Available === "unAvailable"
      );
  ...
  const calculateTimeDiff = (startDate, endDate) => {
    ...
    endDateTime = new Date(
      endDateTime.getTime() - 5 * 60 * 60 * 1000 - 30 * 60 * 1000
    );
    ...
    const intervalId = setInterval(() => {
      SetallContracts((prevContracts) => {
        return prevContracts.map((contract) => {
          return {
            ...contract,
            timeLeft: calculateTimeDiff(
              contract.contract_SD,
  ...
  useEffect(() => {
    ...
    if (statusFilter) {
      ...
      setResults(
        filteredContracts.filter((contract) => contract.Status === statusFilter)
      );
    } else {
    ...
    <div>
      ...
      <div className="w-full h-full py-5">
        ...
        <div className="flex justify-end mb-5">
          <button
            className=" bg-actionBlue text-white px-5 py-2 rounded-lg text-[16px] font-bold  uppercase w-[180px] "
            onClick={() => {
              setopenAddcont(!openAddcont);
            }}
          >
        ...
        <div className=" text-blue-500 font-semibold flex justify-between mb-4">
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            >
              ...
              <option value="waiting for termination">
              ...
              currentItems
                .filter((item) => {
                  ...
                  const firstNameLowerCase =
                    item.clientID.firstName.toLowerCase();
                  const lastNameLowerCase =
                    item.clientID.lastName.toLowerCase();
                ...
                .map((row) => {
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-2 whitespace-nowrap border-r border-gray-200">
                        {calculateTimeDiff(row.contract_SD, row.contract_ED)}
                      </td>
                      <td
                        className={`px-2 text-xs font-semibold text-center ${
                          row.Status === "Newly Added"
                            ? "text-green-500 bg-green-100"
                            : row.Status === "ongoing"
                            ? "text-orange-600 bg-orange-100"
                            : row.status === "waiting for termination"
                      ...
                      <td className="px-6 py-2 whitespace-nowrap justify-center gap-3 flex">
                        <button
                          ...
                          id={row._id}
                          onClick={() => navigate(`/viewContract/${row._id}`)}
                        >
                          View
                        </button>
                        <button
                          className={` ${
                            row.Status === "Terminated" ? "hidden" : ""
                          } bg-actionGreen text-white  py-1 px-6 rounded-md `}
                          onClick={() => navigate(`/EditContract/${row._id}`)}
                        >
                          Edit
                        </button>
                      </td>
        ...
        <div className="flex justify-center mt-4">
          <ul className="flex list-none border border-gray-300 rounded-md">
            {Array.from({
              length: Math.ceil(allContracts.length / itemsPerPage),
            }).map((_, index) => (
              <li
                key={index}
