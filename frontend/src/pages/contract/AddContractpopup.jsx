...
const AddContractpopup = ({isOpen,TogleOpen,clients}) => {
  ...
  return (
    <div className={`${isOpen ? '': 'hidden'} flex flex-col bg-dimWhite fixed left-0 pt-[20px] items-center w-full h-screen  `}>
        ...
        <div className='flex bg-white flex-col ml-[260px] h-fit py-4 px-12 rounded-lg w-[1000px]' onClick={(e) => e.stopPropagation()}>
  ...
  {currentItems ? currentItems
    ...
    .map((row) => {
      return (
        <tr className="bg-white border-t border-gray-200" key={row._id}>
          ...
          <td className="px-6 py-2 whitespace-nowrap justify-center gap-3 flex">
            ...
            <button className="bg-actionGreen text-white py-1 px-6 rounded-md" onClick={()=>{navigate(`/Contract/${row._id}`)}}>Add</button>
          </td>
        </tr>
      );
    }) : (
      ...
      <button className=" bg-orange-600 px-5 py-2 rounded-xl w-[120px] mt-5" onClick={TogleOpen}>
              Close
            </button>
            </div>
