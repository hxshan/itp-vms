...
const ContractEditForm = () => {
  ...
  useEffect(() => {
    if (!ContLoading) {
      if (ContError) {
        ...
        delContract.message === "contract succesfully set to terminated"
      ) {
        alert("Contract Terminated");
        Swal.fire({
          icon: "success",
          title: "Success",
          text: "Contract succesfully Renewed",
          timer: 2000,
    ...
    if (!updLoading) {
      if (updateError) {
        ...
        Swal.fire({
          icon: "warning",
          title: "No changes",
          text: 'No changes detected "Redirecting to contract page',
          timer: 2000,
          showConfirmButton: false,
        }).then(() => {
          navigate(`/viewContract/${contractID}`, {
  ...
  const HandleSubmit = async (e) => {
    ...
    if (!contractData.Vehical_Type) {
      ...
      new Date(contractData.contract_SD).toISOString().split("T")[0] ===
      new Date(contractData.contract_ED).toISOString().split("T")[0]
    ) {
      toast.error(
  ...
  useEffect(() => {
    if (Contract) {
      ...
      if (Contract.Insurance_SD) {
        setInsuranceSD(
          new Date(Contract.Insurance_SD).toISOString().split("T")[0]
        );
      }
      if (Contract.Insurance_ED) {
        setInsuranceED(
          new Date(Contract.Insurance_ED).toISOString().split("T")[0]
        );
      }
      if (Contract.Payment_Date) {
        setpaymentDate(
          new Date(Contract.Payment_Date).toISOString().split("T")[0]
        );
      }
      ...
      if (Contract.contract_ED && Contract.contract_SD) {
        setEstimatedDays(
          calculateDateDiff(
            new Date(Contract.contract_SD),
            new Date(Contract.contract_ED)
          )
        );
      }
  ...
  useEffect(() => {
    ...
    if (contractData.Insurance_SD) {
      setInsuranceSD(
        new Date(contractData.Insurance_SD).toISOString().split("T")[0]
      );
    }
    if (contractData.Insurance_ED) {
      setInsuranceED(
        new Date(contractData.Insurance_ED).toISOString().split("T")[0]
      );
    }
    if (contractData.Payment_Date) {
      setpaymentDate(
        new Date(contractData.Payment_Date).toISOString().split("T")[0]
      );
    }
  }, [contractData]);
          ...
          <div>
            ...
            <div className="  w-fit h-fit  pb-8 rounded-xl ">
              ...
              <div>
                ...
                <button
                  className=" bg-orange-400 text-white px-5 py-2 rounded-lg w-[150px] font-bold mt-4 "
                  onClick={() => {
                ...
                <div className={` ${openTemination ? "" : "hidden"}`}>
                  <button
                    ...
                    className=" bg-red-600 px-5 py-2 rounded-lg font-bold text-white w-[150px] mt-4"
                  >
                    Terminate
                  </button>
                </div>
