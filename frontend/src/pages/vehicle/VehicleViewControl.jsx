...
const VehicleViewControl = () => {
  ...
  <div className="space-y-3 m-1 mt-5 mb-10 p-4  pad shadow-xl bg-white rounded dark:text-white">
      <ToastContainer />  
        ...
        <>
          <div className='flex justify-end'>
            <input
              ...
              className="mb-3 mr-4 shadow appearance-none border rounded py-2 px-3  leading-tight focus:outline-none focus:shadow-outline end-0 2"
            />
            <input
              type="date"
              value={searchTerm}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {filteredAvailable.map((availability, index) => (
          <div key={index} className={`p-4 rounded-md shadow-md ${isDateInRange(new Date(availability.unavailableStartDate), new Date(availability.unavailableEndDate)) ? 'bg-green-400' : 'bg-white'}`}>
            ...
            <p className="text-sm font-semibold  mb-4">Unavailable Start Date:<p className="text-lg font-bold text-black"> {formatDate(availability.unavailableStartDate)} </p></p>
            <p className="text-sm font-semibold  mb-4">Unavailable End Date: <p className="text-lg font-bold text-black"> {formatDate(availability.unavailableEndDate)} </p></p>
          </div>
        ))}
      </div>
    ) : (
